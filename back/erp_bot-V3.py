...
def get_vendor_by_name(name: str) -> str:
    ...
    :return: str vendor details.
    """
    # Construct the URL to fetch vendor details from the mock server (or use a real API call)
    url = f"http://127.0.0.1:8000/vendor?name={name}"

    try:
        # Send a GET request to fetch vendor data
        response = requests.get(url)
        
        # If request was successful, process the data
        if response.status_code == 200:
            return f"{response.json()}"
           
        else:
            return f"Failed to fetch data for {name}. Status Code: {response.status_code}"
...
def get_all_vendors_info(no_parmas: str) -> str:
    ...
    :return: str of html contain vendors' information.
    """
    # Construct the URL to fetch vendor details from the mock server (or use a real API call)
...
tools = [get_document_retriever, get_vendor_by_name , get_all_vendors_info]
...
async def chat(request: ChatRequest):
    ...
    content="""
        ...
        **Using Tools:**  
        ...
        3 Use **`get_document_retriever`** to retrieve specific vendor system-related data.
